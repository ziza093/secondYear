...
a) A function to determine the size of an string.

b) A function to delete 'n' number of characters from a specified position and returns
the adress of the modified string.

c) A function to add an 's2' string in an specified position inside an 's1' string
(It is assumed that there is enough space to fit the 's2' string) and returns
the adress of the 's1' string.

d) A function to check if 2 strings are identical, but with a twist (the 2nd string 
can have an special character '?' which replaces any character).
ex: 'abcde' is identical with '?bc?e'

e) A function to determine whether a word can be found in an array of strings.
